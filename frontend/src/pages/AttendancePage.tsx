...
function AttendanceEditor({ record, onClose, onSaved }: { record: Attendance; onClose: () => void; onSaved: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
    <form onSubmit={save} onClick={(e) => e.stopPropagation()} className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-6 shadow-xl">
      ...
      <div className="mt-6 flex justify-end gap-3"><button type="button" onClick={onClose} className="rounded-lg border px-4 py-2">Cancel</button><button disabled={saving} className="rounded-lg bg-blue-600 px-4 py-2 font-semibold text-white disabled:opacity-60">{saving ? "Saving…" : "Save correction"}</button></div>
    </form>
  </div>;
}

function MedicalAbsenceModal({ employees, onClose, onSaved }: { employees: Employee[]; onClose: () => void; onSaved: () => void }) {
  const [employeeId, setEmployeeId] = useState(""); const [workDate, setWorkDate] = useState(new Date().toISOString().slice(0, 10)); const [notes, setNotes] = useState(""); const [saving, setSaving] = useState(false);
  const save = async (event: React.FormEvent) => { event.preventDefault(); if (!employeeId || !notes.trim()) return toast.error("Employee and medical note are required"); setSaving(true); try { await api.post("/attendance", { employeeId, workDate, notes: `Medical absence: ${notes.trim()}` }); toast.success("Medical absence recorded"); onSaved(); } catch (error: any) { toast.error(error.response?.data?.message || "Could not record absence"); } finally { setSaving(false); } };
...
function AttendanceManagement() {
  ...
  const load = async () => { setLoading(true); try { const response = await api.get("/attendance", { params: { search: search || undefined, date: date || undefined, page, limit: 20 } }); setRecords(response.data.data); setTotal(response.data.total); setTotalPages(response.data.totalPages || 1); } catch (error: any) { toast.error(error.response?.data?.message || "Failed to load attendance"); } finally { setLoading(false); } };
