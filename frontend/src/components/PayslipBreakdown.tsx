            ...
            <div className="p-6 overflow-y-auto space-y-6 flex-1">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 p-4 rounded-xl bg-gray-50/80 border border-gray-200/80">
                    ...
                    <div>
                        ...
                        <div className="text-xs font-medium text-gray-800 flex items-center gap-1.5 mt-0.5">
                            ...
                            {emp.department?.name || contract.department?.name || 'General Operations'}
                        </div>
                        <div className="text-xs text-gray-500 mt-0.5">{contract.jobPosition || 'Employee'}</div>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="text-xs font-medium text-gray-800 flex items-center gap-1.5 mt-0.5">
                            ...
                            {emp.bankName || 'Bank Details Pending'}
                        </div>
                        <div className="text-xs font-mono text-gray-500 mt-0.5">
                            {emp.bankAccountNo ? `Acc: ••••${String(emp.bankAccountNo).slice(-4)}` : 'No account standard'}
                        </div>
                    </div>

                    <div>
                        <span className="text-[11px] font-bold uppercase tracking-wider text-gray-400 block mb-0.5">Contract Base Wage</span>
                        <div className="text-sm font-bold text-gray-900">{fmtCurrency(contract.wage || payslip.basicAmount)}</div>
                        <div className="text-[11px] text-emerald-600 font-semibold mt-0.5">
                            Worked Days: {payslip.workedDays || 0}
                        </div>
                    </div>
                </div>
                                            ...
                                            {line.explanation && (
                                                <div className="relative inline-block">
                                                    ...
                                                    {activeTooltip === line.code && (
                                                        <div className="absolute left-0 bottom-full mb-1 z-30 w-64 p-2.5 rounded-lg bg-slate-900 text-white text-[11px] font-mono leading-tight shadow-xl">
                                                            {line.explanation}
                                                        </div>
                                                    )}
                                                </div>
                ...
                <div className="p-5 rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-700 text-white shadow-lg flex flex-wrap items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="text-xs text-emerald-200 mt-0.5">Calculated Net = Gross Earnings (₹{grossCalculated.toLocaleString('en-IN')}) – Total Deductions (₹{deductionTotal.toLocaleString('en-IN')})</p>
                    </div>
                ...
                <div className="rounded-xl border border-gray-200 bg-gray-50/50 overflow-hidden">
                    ...
                    >
                        <div className="flex items-center gap-2">
                            ...
                            <span className="font-bold text-sm text-gray-900">Explain Calculation Chain & Rule Formula Flow</span>
                        </div>
                    ...
                    {showExplanationChain && (
                        <div className="p-4 border-t border-gray-200 bg-white space-y-3 font-mono text-xs">
                            <p className="text-gray-500 font-sans text-xs mb-3">
                                Detailed step-by-step resolution executed by the Rule Engine in sequence order:
                            </p>
                            ...
                            {sortedLines.map((line, idx) => (
                                <React.Fragment key={line.id || idx}>
                                    <div className="p-3 rounded-lg bg-gray-50 border border-gray-200 flex flex-col md:flex-row md:items-center justify-between gap-2">
                                        ...
                                        <div className="text-gray-600 bg-white px-2.5 py-1 rounded border border-gray-200 max-w-full text-ellipsis overflow-hidden">
                                            {line.explanation || `Category: ${line.category} | Amount: ${line.amount}`}
                                        </div>
                                    </div>
